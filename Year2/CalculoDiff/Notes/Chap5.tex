...
Tenemos que $\displaystyle \nabla f\left(x,y\right) = \left(2,3\right) \neq \left(0,0\right) $ y $\displaystyle f $ es lineal. El problema se puede ver de más sencillamente de forma geométrica. Buscamos el punto $\displaystyle \left(x,y\right) \in E $ tal que $\displaystyle \nabla F\left(x,y\right) = \nabla f\left(x,y\right) $, es decir, existe $\displaystyle \lambda \in \R $ con 
...
\begin{definition}[Variedades diferenciables implícitas]
	Sean $\displaystyle 1\leq k \leq n $; $\displaystyle U \subset \R^{n} $ abierto, $\displaystyle F : U \to \R^{k} $ de clase $\displaystyle \mathcal{C}^{m} $. Consideramos el conjunto 
...
\begin{eg}
\begin{enumerate}
	\item Calcular el máximo y el mínimo de $\displaystyle f\left(x,y,z\right) = 2x-z^{2} $ en la esfera $\displaystyle S = \left\{ \left(x,y,z\right) \; : \; x^{2} +y^{2} + z^{2} = 5\right\}  $. Como $\displaystyle f $ es continua y $\displaystyle S $ es continuo, existen $\displaystyle p_{m}, p_{M} \in S $ tales que 
