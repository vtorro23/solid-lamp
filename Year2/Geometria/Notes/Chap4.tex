...
\begin{proof}
	...
	y en el caso que $\displaystyle p < r-p $ podemos multiplicar por $\displaystyle -1 $. Por tanto, $\displaystyle \left[F\right]  = \left[x_{0}^{2}+ \cdots +x_{p-1}^{2} -x_{p}^{2}-\cdots -x_{r}^{2}\right]  $ 
...
\[F = 10x_{0}^{2} + 2x_{0}x_{1} + x_{1}^{2} + 4x_{2}^{2} = 9x_{0}^{2} + \left(x_{0}+x_{1}\right)^{2}+4x_{2}^{2} .\]
...
\end{observation}
\begin{eg}
Consideremos $\displaystyle f = 2x_{1}^{2}-x_{1}x_{2}+4x_{1}-6x_{2}-x_{2}^{2} $. Tenemos que 
