...
Por tanto, como esto es cierto $\displaystyle \forall \epsilon > 0 $, se tiene que 
\[ \int^{b}_{a} f = \int^{c}_{a} f +\int^{b}_{c} f .\]
\end{proof}
\begin{observation}
...
\item[(d)] $\displaystyle \log_{a} y= \frac{\log_{x}y}{\log_{x}a} $.
...
\begin{ftheorem}[Teorema de integrabilidad de Lebesgue]
	\normalfont Sea $\displaystyle f : \left[a,b\right] \to \R $ una función acotada. Tenemos que $\displaystyle f $ integrable en $\displaystyle \left[a,b\right]  $ si y solo si el conjunto de discontinuidades de $\displaystyle f $ tiene contenido cero.
\end{ftheorem}
\begin{fprop}[]
